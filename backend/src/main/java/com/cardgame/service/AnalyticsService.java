...
public class AnalyticsService implements Serializable {
    ...
    public List<CardVO> cards(final UUID gameId, final UUID playerId) throws InvalidGameIDException, EmptyGameDeckException {
            ...
            if (Objects.nonNull(game.getDeck()) && !game.getDeck().isEmpty()) {
                return game.getDeck().stream()
                    ...
                    .map(c -> {
                        ...
                        vo.setId(c.getId());
                        vo.setFace(c.getFace());
                        vo.setSuit(c.getSuit());
                        return vo;
                    })
