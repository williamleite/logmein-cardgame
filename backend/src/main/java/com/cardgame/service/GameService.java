...
public class GameService implements Serializable {
    ...
    @Transactional
    public CardVO deal(UUID gameId, UUID playerId) throws InvalidGameIDException, EmptyGameDeckException, InvalidPlayerIDException, PlayerNotInGameException, EmptyGamePlayersException {
        Game game = this.repository.findById(gameId).orElse(null);
        if (Objects.nonNull(game)) {
                    ...
                    if (Objects.nonNull(player)) {
                        if (Objects.nonNull(game.getPlayers()) && !game.getPlayers().isEmpty()) {
                            if (game.getPlayers().stream().anyMatch(p -> Objects.equals(p.getId(), playerId))) {
                                ...
                                available = this.cardRepository.saveAndFlush(available);
                                
                                CardVO result = new CardVO();
                                result.setFace(available.getFace());
                                result.setId(available.getId());
                                result.setSuit(available.getSuit());
                                return result;
                            } else {
